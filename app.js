...
async function initApp() {
    ...
    try {
        ...
        await loadDemoData();
        setupEventListeners();
        if (typeof checkCurrentShift === 'function') { await checkCurrentShift(); } loadModule('productos');
    } catch (error) {
...
async function dbAdd(store, data) {
    if (useLocalStorageFallback) {
        ...
        idCounter++; data.id = Date.now() + idCounter;
        items.push(data);
...
function loadModule(module) {
    ...
    switch(module) {
        ...
        case 'bodega': loadIngredients(); loadRecipes(); loadSuppliers(); loadBatches(); if (typeof loadBodegaReports === 'function') loadBodegaReports(); break;
    }
}
    ...
    try {
        ...
        grid.innerHTML = filtered.map(product => `
            <div class="product-card" onclick="addToCart(${product.id})" data-id="${product.id}">
                <div class="product-image">${product.image ? `<img src="${product.image}" alt="${product.name}" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML='<div style=font-size:48px>🧋</div>'">` : '<div style="font-size:48px">🧋</div>'}</div>
...
async function processCheckout() {
    ...
    try {
        ...
        // FIX #13: Verificar stock de TODOS los productos ANTES de descontar
        const stockChecks = [];
        for (const cartItem of cart) {
            ...
            if (product.stock < cartItem.qty) {
                showToast(`❌ Sin stock: ${product.name} (disponible: ${product.stock})`, 'error');
                return;
            }

            // Verificar ingredientes de bodega
        ...
        // FIX #13: Si pasó todas las verificaciones, ahora sí descontar
        const bodegaUpdates = [];
        for (const cartItem of cart) {
            const product = products.find(p => p.id === cartItem.id);
            if (!product) continue;
        ...
        // Fiado: procesado más abajo en el bloque de pago
        ...
        if (currentPaymentMethod === 'efectivo') {
            const received = prompt(`💵 Total: ${formatMoney(total)}\n¿Cuánto recibió el cliente?`) || total;
            const receivedNum = parseFloat(received) || total;
            amountPaid = receivedNum;
            change = Math.max(0, receivedNum - total);
            if (change > 0) showToast(`💰 Vuelto: ${formatMoney(change)}`, 'success');
        } else if (currentPaymentMethod === 'tarjeta') {
            amountPaid = total;
            change = 0;
            showToast(`💳 Tarjeta: ${formatMoney(total)}`, 'success');
        } else if (currentPaymentMethod === 'transferencia') {
            amountPaid = total;
            change = 0;
            showToast(`🔄 Transferencia: ${formatMoney(total)}`, 'success');
        } else if (currentPaymentMethod === 'fiado') {
            amountPaid = 0;
            change = 0;
            const clients = await dbGetAll(STORE_CLIENTS);
            if (clients.length === 0) { showToast('❌ No hay clientes', 'error'); return; }
            const clientPrompt = prompt(`Cliente para fiado:\n${clients.map((c, i) => `${i + 1}. ${c.name}`).join('\n')}\nNúmero:`);
            const clientIndex = parseInt(clientPrompt) - 1;
            if (clientIndex >= 0 && clientIndex < clients.length) {
                ...
                showToast('❌ Cliente inválido', 'error'); return;
            }
        }

        // Guardar venta PRIMERO
        const saleId = await dbAdd(STORE_SALES, { items, subtotal, total, amountPaid, change, paymentMethod: currentPaymentMethod, clientId, clientName, date: new Date().toISOString(), timestamp: Date.now() });

        // Si es fiado, guardar deuda vinculada
        let debtId = null;
        if (currentPaymentMethod === 'fiado' && clientId) {
            debtId = await dbAdd(STORE_DEBTS, {
                clientId, clientName, saleId, amount: total, paid: 0, remaining: total,
                installments: 1, status: 'pending', description: `Venta fiada: ${items.map(i => i.name).join(', ')}`,
                dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
                createdAt: new Date().toISOString()
            });

            // Actualizar venta con debtId
            ...
            if (client) {
                client.totalDebt = (client.totalDebt || 0) + total;
        ...
        if (typeof addSaleToShift === 'function') await addSaleToShift(total, amountPaid, currentPaymentMethod);
...
async function loadSales() {
    try {
        ...
        list.innerHTML = sales.map(sale => {
            ...
            const itemNames = safeItems.map(i => i.name || 'Producto').join(', ') || 'Venta rápida';
            const dateStr = formatDate(sale.date);
            const isFiado = sale.paymentMethod === 'fiado';
            return `<div class="list-item"><div class="list-icon">${icons[sale.paymentMethod] || '💰'}</div><div class="list-content"><div class="list-title">${itemNames} ${isFiado ? '<span style="background:rgba(255,217,61,0.2);color:var(--accent);padding:2px 6px;border-radius:8px;font-size:11px;">FIADO</span>' : ''}</div><div class="list-subtitle">${dateStr} • ${safeItems.length} productos</div></div><div style="display:flex;align-items:center;gap:8px;"><div class="list-amount">${formatMoney(sale.total)}</div><button class="action-btn delete" onclick="deleteSale(${sale.id})" title="Eliminar venta">🗑️</button></div></div>`;
        }).join('');
    } catch (error) {
...
async function deleteSale(id) {
    showConfirm('¿Eliminar esta venta? Se revertirá el stock.', async () => {
        try {
            ...
            // Si era fiado, eliminar deuda asociada
            if (sale.debtId) {
                await dbDelete(STORE_DEBTS, sale.debtId);
                // Revertir deuda del cliente
                if (sale.clientId) {
                    const client = await dbGet(STORE_CLIENTS, sale.clientId);
                    if (client && client.totalDebt) {
                        client.totalDebt = Math.max(0, client.totalDebt - sale.total);
                        await dbPut(STORE_CLIENTS, client);
                    }
                }
            }
...
console.log('✅ Colibrí Boba Tea v4.3 cargado completamente - 20 bugs fixeados');
